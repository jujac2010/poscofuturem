...
export type ForkliftDetailProps = {
  ...
  assessment: MaintenanceUiAssessment | null;
  onSubmitAction: (input: CreateMaintenanceAction, status: ActionStatus) => Promise<void>;
};
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (status !== action.status) {
        ...
        finalStatus = (await update.json())?.status ?? status;
      }
      setSaved(finalStatus);
      await onSubmitAction(input, finalStatus);
    } catch (submitError) {
